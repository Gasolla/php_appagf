...
<div  class="table-responsive class-grid" id="listacliente" >
    <table border="0" width="100%" class="table-bordered">
        ...
        <tbody>
            ...
            $contzebra = 0;
            if ($sucesso===true) {
                foreach ($controller->getEstoquecliente() as $key => $value) {
                    if($contzebra % 2 == 0){
                    ...
                    <tr id='<?php echo crypto::encrypt($value->getId()) ?>'> 
                        <td class='text-center td-grid' <?php echo $color ?>>
                            ...
                            <a onclick="ConfirmaExcluir('<?php echo $pagina ?>', '<?php echo crypto::encrypt($value->getId()) ?>', 'Deseja excluir o estoque do cliente '+'<?php echo $value->getCliente() ?>'+'?')" 
