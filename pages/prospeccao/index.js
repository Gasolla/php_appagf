...
Gravar = (formulario) => {
    for (var i = 0; i < formulario.length; i++) {
        if ((formulario[i].name === 'nome') &&
            ...
            erroElemento(formulario[i]);
            return false;
        } else if ((formulario[i].name === 'contato') &&
                (formulario[i].value === '') && wcontato) {
            erroElemento(formulario[i]);
            return false;
        } else if ((formulario[i].name === 'ramo') &&
                (formulario[i].value === '') && wcontato) {
            erroElemento(formulario[i]);
            return false;
        } else if ((formulario[i].name === 'volume') &&
                (formulario[i].value === '') && wcontato) {
            erroElemento(formulario[i]);
            return false;
        } else if ((formulario[i].name === 'datacontato') &&
                (formulario[i].value === '')) {
            erroElemento_msg(formulario[i], 'Campo data contato inválido');
            return false;
        }else if ((formulario[i].name === 'telefone') &&
                (formulario[i].value === '')) {
            erroElemento(formulario[i]);
            return false;
        }else if ((formulario[i].name === 'comentario') &&
                (formulario[i].value === '')) {
            erroElemento(formulario[i]);
            return false;
        }
...
seleciona_motivo = (ch, id, rad) => {
    contato = (rad.value!="R");
    document.getElementById(id).classList.add((ch === false)?'d-none':'d-flex');
    document.getElementById(id).classList.remove((ch === false)?'d-flex':'d-none');       
};