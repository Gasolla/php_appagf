    ...
    <table border="0" width="100%" class="table-bordered">
        ...
        <thead>
            <tr>
                ...
                <th scope="col" class='text-left th-grid'>USUÁRIO</th>
        ...
        <tbody>
            ...
            $contzebra = 0;
            if ($sucesso===true) {
                foreach ($controller->getUsuario() as $key => $value) {
                    if($contzebra % 2 == 0){
                    ...
                    <tr id='<?php echo crypto::encrypt($value->getId()) ?>'> 
                        <td class='text-center td-grid' <?php echo $color ?>>
                            ...
                            <a onclick="ConfirmaExcluir('<?php echo $pagina ?>', '<?php echo crypto::encrypt($value->getId()) ?>', 'Deseja excluir o usuário '+'<?php echo $value->getNome() ?>'+'?')" 
